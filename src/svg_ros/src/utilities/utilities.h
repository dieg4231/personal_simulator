...
int GetEnviromentFlags(int *sensorflag,int *lightflag,int *baseflag,int *invertflag){
  static ros::NodeHandle n;
  static ros::ServiceClient client;
  static svg_ros::EnvironmentSrv srv;
  char param[]="Send flags";
  client = n.serviceClient<svg_ros::EnvironmentSrv> ("environment");
  srv.request.param=param;

  if (client.call(srv))
  {
    printf("\nFlags:\n \t Sensor:%d  Light:%d   Base:%d   Invert: %d",srv.response.sensor,srv.response.light,srv.response.base,srv.response.invert);
  ...
  {
    printf("Failed to call service send_position");
    return 1;
  }
...
int GetObservations(coord* coord_robot, Raw* observations,float largest_value,int sensorflag,int invertflag){

  static ros::NodeHandle n;
  static ros::ServiceClient client;
  static svg_ros::SensorSrv srv;
  static int flg = 1,j,i;
 ...
 // if(flg == 1){
  ...
  if(sensorflag && invertflag)
    for( j=0, i=srv.response.num_sensors-1;j<srv.response.num_sensors;j++,i--){
      if (srv.response.sensors[j]>largest_value)
          observations->sensors[i]=largest_value;
      else
        observations->sensors[i]=srv.response.sensors[j];
      printf("observations->sensors[%d] %f srv.response.sensors[%d] %f\n",j,observations->sensors[j],j,srv.response.sensors[j]);
      }
  else
    for( j=0;j<srv.response.num_sensors;j++){
      if (srv.response.sensors[j]>largest_value)
...
float angle_goal(coord coord_robot,coord coord_dest){
  ...
  /*if ( angle < PI/4 || (angle > 7*PI/4  && angle < 2*PI) ) value=5;
  else if( angle < PI / 2) value = 3;
  else if ( angle < PI ) value = 1;
  else if( angle < 3*PI/2 ) value = 0;
  else value = 2;
*/
