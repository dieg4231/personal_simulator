...
#include "../state_machines/surround.h"
...
//get inputs from GUI
 ...
 GetEnviromentFlags(&sensorflag,&lightflag,&baseflag,&invertflag);
 while(flg == 0){
	...
	GetObservations(&coord_robot,&observations,largest_value,sensorflag,invertflag);
	...
	switch ( selection) {
		...
		case 4:
			potential_fields(coord_robot ,coord_dest,observations,inputs.num_sensors,largest_value,inputs.theta_sensor,inputs.range_sensor,&DistTheta.angle);
			DistTheta.distance=inputs.Mag_Advance;
		break;
		case 5:
			surround(coord_robot ,coord_dest,observations,inputs.num_sensors,largest_value,inputs.theta_sensor,inputs.range_sensor,&DistTheta.angle);
			DistTheta.distance=inputs.Mag_Advance;
		break;
	...
	//it moves the robot
	mvrobot(DistTheta,&coord_robot,baseflag);
